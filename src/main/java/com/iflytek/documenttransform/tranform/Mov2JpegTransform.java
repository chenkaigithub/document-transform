 ...
 * @date 2016年6月17日
 */
public class Mov2JpegTransform implements Transform {
    private static final Logger LOGGER            =
            LoggerFactory.getLogger(Mov2JpegTransform.class);
    ...
    public String transform(String movFileId, Map<String, Object> parameter) {
        ...
        try {
            ...
            byte[] flvFileData = FastdfsFileServer.INSTANCE.get(movFileId);

            movFileName = FastdfsFileServer.INSTANCE.fileMateValue(movFileId,
                    Constant.FASTDFS_FILENAME_KEY);

            FileUtils.writeByteArrayToFile(movFileTemp, flvFileData);
            ...
            // /Users/suenlai/Downloads/ffmpeg -i mp42.mov -f image2 -ss 3 -t 1 test.jpeg
            // -r 1 每秒一张
            // -t 截图1移
            // -ss 截图开始时间
            ...
            jpegFileId = FastdfsFileServer.INSTANCE.saveUploadFile(fis,
                    FileNameUtils.changeFileExt(movFileName, "jpeg"));
            LOGGER.info("转换MOV[fileId=" + movFileId + "]到JPEG[fileId=" + jpegFileId + "]成功");
            return jpegFileId;
        } catch (IOException e) {
            LOGGER.error("转换MOV[fileId=" + movFileId + "]到JPEG[fileId=" + jpegFileId + "]失败,请查看是否因为截图时间戳超过此视频的时长", e);
            throw new TransformException("转换FLV到JPEG失败", e);
        } catch (InterruptedException e) {
            LOGGER.error("转换MOV[fileId=" + movFileId + "]到JPEG[fileId=" + jpegFileId + "]失败", e);
